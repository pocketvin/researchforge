...
- Phase 5 product hardening: **engineering checkpoint passed locally and in public branch CI**.
- Published Phase 5 candidate CI: **passed** on commit `5062370` in run
  [33809641523](https://github.com/pocketvin/researchforge/actions/runs/33809641523); backend,
  frontend and container jobs are green.
...
- [Phase 5 evidence](docs/evidence/v1.5-product-hardening/README.md) is automated engineering
  evidence only. The exact candidate passed all public GitHub Actions jobs.
...
Phase 1 remains closed. Phases 2–5 are closed as engineering checkpoints. Phase 6 is ready for
owner-coordinated recruitment but has not started: zero real-human sessions exist. The next and
only remaining independent review occurs once at the final Phase 7 release gate.
...
Arrange at least six consenting real target users, then run the already frozen counterbalanced
Web+n8n protocol without changing its denominator or thresholds. Until real sessions exist,
Phase 6 remains `not_started`, human usefulness remains `UNVALIDATED`, and Release Freeze cannot
begin. No intermediate independent acceptance is required.
