...
- The current full gate passes 153 backend tests across 73 typed source files; primary/contingency packages, calibration/formal engines, simulation executor, API, storage, security boundaries, formulas, and Final Test controls are covered.
...
- Primary 24-case package is owner-signed: 24 official Source Documents, 144 normalized Financial Facts, 24 explicitly synthetic public Evidence Chunks, and 24 cases split 12/6/6. All 96 newly acquired fact cells were page-matched; formal execution is authorized while Final Test remains sealed by protocol.
...
- Calibration preflight is independently bounded to one synthetic request and USD 0.0064 worst case under the USD 1 allocation. It blocks before provider client construction when the rotated-key marker is absent.
- Zero-network formal preflight reports a USD 1.8432 maximum reservation for all 288 possible requests including one repair per run, below both the USD 9 primary allocation and USD 20 aggregate cap; it now also requires a valid pinned calibration artifact.
...
Place a new rotated key only in the created, ignored local `.env` and set `RESEARCHFORGE_ROTATED_KEY_CONFIRMED=1`; then pass the one-request synthetic calibration and run the formal primary experiment without changing model, graph, data, Verifier, or thresholds.
...
PASS: 69 locked packages, 117 formatted files, lint, strict mypy over 73 source files, 153/153 tests, 85 required contract files, diff whitespace, and Compose configuration.
...
PASS locally: 24 sources, 144 facts, 24 synthetic chunks, 24 frozen cases; package hash 3638eb1ca7b8192cb6a901f4b0d51c8373ccaff1e776758605f1d4b975cb1c3f; owner-signed at 2026-09-01T19:14:33+08:00, Final Test still sealed.

Formal experiment executor
PASS locally with a synthetic test double: 144/144 temporary runs and artifacts validated. Primary preflight also validates the signed package and sealed contingency commitment. Real preflight is BLOCKED before provider contact only by rotated-key confirmation and the derived missing-calibration gate. OpenAI spend remains USD 0.

Provider calibration gate
PASS offline controls: exactly one synthetic non-research context, fixed model/configuration, full seven-code coverage, idempotent artifact recovery, USD 0.0064 worst case and no client construction before prerequisites pass. Live calibration remains BLOCKED before provider contact by rotated-key confirmation.
