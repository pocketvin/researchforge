...
- P1 reliability hardening after the second code audit is implemented without changing research methodology: provider-budget reservations are now 300-second crash leases rather than permanent restart ghosts; verified cache fallback requires the current parser version; terminal Trace/SSE drains the complete durable event journal rather than truncating after 2,000 events; `/v2/capabilities` reports the actual DeepSeek semantic-review fallback separately from Qwen fallback Reflection/Synthesis; and malformed or market-incompatible report periods fail request validation before a Run is created. Focused regression coverage is included in the **302/302** project test gate.
