# ResearchForge V1.7.1 Final Delivery Roadmap

**Updated:** 2026-09-05
**Status:** V1.7.1 engineering complete; owner re-acceptance remains

## Final product definition

ResearchForge V1.7.1 is an auditable, question-driven Financial Research Agent with explicit Research Synthesis vs Evidence Summary modes:
...
- Surfaces: Web and n8n V1.7.1 presentation over the same authoritative backend; stable V17 routes/IDs are retained for compatibility.
...
Final verified counts: 210 pytest tests, 105 mypy source files, 5 frontend unit tests, 3 mocked E2E, 3 live-backend E2E, 11 n8n Node tests, 3 Docker smoke cases, 3 actual n8n success cases and 5 transport-only failure scenarios.

## Phase F — Owner re-acceptance / Release Freeze

**Status: pending owner re-acceptance; engineering is complete.**

The first V1.7 Owner Acceptance failed on synthesis quality and is preserved as such. The owner now manually rechecks representative arbitrary-company model synthesis, the explicit fallback state, Evidence/Trace readability and one bounded failure. Automation must not invent this human acceptance. There is no six-person Human Pilot prerequisite.
