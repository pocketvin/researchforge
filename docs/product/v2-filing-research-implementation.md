...
The primary hybrid route uses DeepSeek V4 Flash for the high-frequency filing Tool Loop, public-state Reflection and structured Synthesis; Qwen Plus performs claim-wise semantic review and Qwen3-VL Plus is selected only for evidence that actually requires page-image understanding. Kimi remains optional standby metadata only and is not required for the active route. Provider-private histories do not cross provider boundaries. DeepSeek availability is not a single point of failure: timeout/connection or HTTP 402/408/429/5xx triggers a visible, run-sticky fallback to Qwen Plus for Tool Calling and Qwen3-Max for Reflection/Synthesis, while claim-wise Review remains Qwen Plus. HTTP 400 is deliberately not hidden by fallback.
