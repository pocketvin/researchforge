"""Regression tests for the sealed V1.5 contingency benchmark package."""
...
PACKAGE_DIR = ROOT / "data" / "fixtures" / "v1.5-contingency"
PRIMARY_DIR = ROOT / "data" / "fixtures" / "v1.4-primary"
...
def test_contingency_shape_split_isolation_and_primary_disjointness() -> None:
    ...
    assert not (groups["evolution"] & groups["validation"])
    assert not (groups["evolution"] & groups["final_test"])
    assert not (groups["validation"] & groups["final_test"])
    primary_groups = {
        load_json(path)["group_key"] for path in (PRIMARY_DIR / "cases").glob("*.json")
    }
    assert not (set().union(*groups.values()) & primary_groups)
    assert not list(PACKAGE_DIR.rglob("*.pdf"))


def test_contingency_sources_and_facts_retain_lineage_and_semantics() -> None:
    ...
    for document_id, source in SOURCES.items():
        assert urlparse(source["source_uri"]).netloc == "static.cninfo.com.cn"
...
def test_contingency_cases_bind_one_report_and_obey_cutoff() -> None:
...
def test_contingency_public_evidence_is_explicitly_synthetic() -> None:
...
def test_contingency_hashes_are_stable_and_activation_is_disabled() -> None:
    public_paths = sorted(
        path
    ...
    assert MANIFEST["public_artifact_hashes"] == expected_hashes
    data_hashes = {
    ...
    assert MANIFEST["evidence_status"] == "FROZEN_CONTINGENCY_SEALED"
    assert MANIFEST["formal_run_authorized"] is False
    assert MANIFEST["contingency_activation_authorized"] is False
    assert MANIFEST["sealed_until"] == "PRIMARY_VALIDATION_REJECTS_CANDIDATE"
