# ResearchForge V1.8.5 n8n Integration
...
## V1.8.5 presentation on the stable V17 workflow
...
The native form uses the V1.8.5 product presentation over the preserved V1.7 General Research backend contract. Automation webhook callers may additionally send `research_mode: "financial_snapshot"` for the bounded reviewed-cache compatibility path used by deterministic packaging/CI smoke; ordinary webhook requests default to `general`.
...
The generator builds the V1.8.5 presentation on the stable V17 workflow from reviewable Code-node source files under `integrations/n8n/code/`.
...
docker compose -f docker-compose.yml -f integrations/n8n/compose.yml \
  ...
  import:workflow --input=/files/researchforge-v1.7.workflow.json
...
V1.8.5 transport presentation still returns the preserved V1.7 transport output (schema 1.7.0) is validated by `schemas/v1.7/n8n-research-output.schema.json`. V1.6 and V1.5 n8n artifacts keep their original schemas and remain historical evidence.
