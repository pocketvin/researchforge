...
node('Respond', 'respondToWebhook', 1.4, { respondWith: 'json', responseBody: '={{ $json }}',
  options: { responseCode: '={{ $json.http_status }}' } }, [4200, 0]);
const chain = ['Research webhook', 'Prepare request', 'Input valid', 'Check product backend',
  ...
  'Map verified output', 'Respond'];
for (let i = 0; i < chain.length - 1; i++) connect(chain[i], chain[i + 1]);
for (const name of ['Input valid', 'Backend ready', 'Submission accepted']) connect(name, 'Respond', 1);
connect('Run completed', 'Keep waiting', 1);
connect('Keep waiting', 'Wait before polling');
connect('Keep waiting', 'Respond', 1);
